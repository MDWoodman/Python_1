...
class ichimoku_object:
    ...
    def analyze_ichimoku(self, stock_data_in, last_n_candles, tenkansen_period, kiusen_period, senokuspanB_period):
         
        stoc_data = self.calculate_ichimoku(stock_data_in , tenkansen_period , kiusen_period , senokuspanB_period)
        last_n_candles_df = self.get_data_from_candle_array(last_n_candles)
        ...
        crossover_price_kiusen_result , time_of_cross_price_kiusen = self._check_crossover_price_kiusen(last_n_candles_df, list_of_data_kiusen)
        ...
        crossover_price_senokuspan , time_of_cross_price_senokuspan =   self._check_crossover_price_senokuspan_a_senokuspan_b(last_n_candles_df, list_of_data_senkou_span_a, list_of_data_senkou_span_b , list_of_data_time)
    ...
    def _check_crossover_price_senokuspan_a_senokuspan_b(self, last_n_candles, senkou_span_a, senkou_span_b , list_of_data_time):
        ...
        import tools
        result = []
        if len(last_n_candles) == 0:
            return "Brak przecięcia", None
        
        for i in range(len(last_n_candles) - 1, 0, -1):
            row = last_n_candles.iloc[i]
            candle_time = int(row["Date"])
            candle_open = float(row["Open"])
            candle_close = float(row["Close"])
            idx=0
            for j in range(len(list_of_data_time) - 1, 0, -1):
                if candle_time == list_of_data_time[j]:
                    idx = j
                    break
            if pd.isna(senkou_span_a[idx]) == False:
                if candle_open > senkou_span_a[idx] and candle_close < senkou_span_a[idx]:
                    result.append(str(i) + ", Cena przecina do dołu SenkouSpanA" +"," + str(candle_time) + ", " + str(tools.int_to_datetime(candle_time)))
                elif candle_open < senkou_span_a[idx] and candle_close > senkou_span_a[idx]:
                    result.append( str(i) + ", Cena przecina do góry SenkouSpanA" + "," + str(candle_time) + ", " + str(tools.int_to_datetime(candle_time)))
        ...
        for i in range(len(last_n_candles) - 1, 0, -1):
            ...
            if pd.isna(senkou_span_b[idx]) == False:
                if candle_open > senkou_span_b[idx] and candle_close < senkou_span_b[idx]:
                    result.append(str(i) + ", Cena przecina do dołu SenkouSpanB"  +","+ str(candle_time) + ", " + str(tools.int_to_datetime(candle_time)))
                elif candle_open < senkou_span_b[idx] and candle_close > senkou_span_b[idx]:
                    result.append( str(i) + ", Cena przecina do góry SenkouSpanB" + ","+ str(candle_time )+ ", " + str(tools.int_to_datetime(candle_time)))
        ...
        return "Brak przecięcia", int(last_n_candles.iloc[0]["Date"])
        ...
        for j in range(len(kiusen) - 1, 0, -1):
            for i in range(len(last_n_candles) - 1, 0, -1):
                ...
                if(pd.isna(kiu) == False):
                    if candle_open > kiu and candle_close < kiu:
                        return ichi_crossover_price_kiusen_result_enum.Przeciecie_do_dolu, candle_time
                    elif candle_open < kiu and candle_close > kiu:
                        return ichi_crossover_price_kiusen_result_enum.Przeciecie_do_gory, candle_time
            
 
        return ichi_crossover_price_kiusen_result_enum.Brak_przeciecia , int(last_n_candles.iloc[0]["Date"])
