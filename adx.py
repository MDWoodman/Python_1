...
#  stock_data.set_index('Date', inplace=True)
    ...
    def analyze_adx(self ,stock_data_in, adx_conf , lookback=14):
        ...
        time_in = stock_data_in['Date']

        plus_di, minus_di, adx ,time = self.get_adx(high, low, close,time_in ,lookback)
    
        length = len(plus_di)
        list_of_data_plus_di = []
        for i in range(length):
            list_of_data_plus_di.append(plus_di.iloc[-i])

        list_of_data_plus_di.reverse()

        list_of_data_minus_di = []
        for i in range(length):
            list_of_data_minus_di.append(minus_di.iloc[-i])
        
        list_of_data_minus_di.reverse()

        list_of_data_adx = []
        for i in range(length):
            list_of_data_adx.append(adx.iloc[-i])

        list_of_data_adx.reverse()

        list_of_data_time = []
        for i in range(length):
            list_of_data_time.append(time.iloc[-i])

        list_of_data_time.reverse()

        for i in range(len(list_of_data_adx)):
          for i in range(len(list_of_data_adx)):
            try:  
                if math.isnan(list_of_data_adx[i]):
                    list_of_data_adx.pop(i)
                    list_of_data_minus_di.pop(i)
                    list_of_data_plus_di.pop(i)
                    list_of_data_time.pop(i)
            except:
                continue
        ...
        if crossover_point is not None:
            ...
            if end_cross >= len(list_of_data_plus_di):
                 end_cross = len(list_of_data_plus_di) -1
           

            list_of_data_plus_di_cross = list_of_data_plus_di[start_cross:end_cross+1]
            list_of_data_minus_di_cross = list_of_data_minus_di[start_cross:end_cross+1]
            
            crossover_result = self._check_crossover_v2(list_of_data_plus_di_cross, list_of_data_minus_di_cross)
